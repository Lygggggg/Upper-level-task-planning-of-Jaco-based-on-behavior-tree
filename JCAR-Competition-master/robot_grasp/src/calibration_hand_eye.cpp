...
typedef Eigen::Matrix<float,3,3> Matrix3_;
using namespace cv;
using namespace std;
...
Matrix3_ rotate_mat;
;
Eigen::AngleAxis<float> rotate_axis;
void img_callback(sensor_msgs::Image msg)
{
  ...
  found = findChessboardCorners(view, board_size, image_points_buf,cv::CALIB_CB_ADAPTIVE_THRESH | cv::CALIB_CB_FAST_CHECK | cv::CALIB_CB_NORMALIZE_IMAGE);
  if (found)
  {

...
//    threePyramid_param
    ...
    //ros::ServiceServer service = nh.advertiseService("/get_pick_place_routine", &PickPlace::kinova_routine, this);
    //my_pick();
}
...
void PickPlace::define_cartesian_pose()
{
	...
	obj_place_pose.pose.position.z = 0.012587;
	...
	pre_obj_place_pose.pose.position.z = obj_place_pose.pose.position.z +0.15;
	
	new_obj_place_pose = obj_place_pose;
	new_obj_place_pose.pose.position.x = obj_place_pose.pose.position.x - 0.03;
	new_obj_place_pose.pose.position.y = obj_place_pose.pose.position.y - 0.05;
	new_obj_place_pose.pose.position.z = obj_place_pose.pose.position.z + 0.12;*/
  ...
  if (close_gripper)
  {
    ...
    switch (num_of_label)
    {
      ...
      //"cube" ok
        ...
        gripper_joint_positions[2] = 0.965;//90
        ROS_INFO("closed");
        break;
      }
      case 2:
      {
      //"cone" ok
        /*gripper_joint_positions[0] = 1.11;
        gripper_joint_positions[1] = 1.11;//89
        gripper_joint_positions[2] = 1.11;//90*/
...
bool PickPlace::kinova_routine(robot_grasp::G_pick_place::Request &req,robot_grasp::G_pick_place::Response &res)
{
  ...
  //group_->setPlanningTime(10);
	...
	//detected_param_obj_list = {0.240161,-0.168802,0.731732};
  ...
  /*for(int i=0;i<detected_param_obj_list.size();i++)
  {
	...
	switch(num_of_label)
	{
		...
		case 9://"sixPrism"
			{
				obj_place_pose.pose.position.x = 0.51123;
    			obj_place_pose.pose.position.y = -0.21853;
				break;
			}
		case 10://"threePrism"
			{
				obj_place_pose.pose.position.x = 0.5483;
    			obj_place_pose.pose.position.y = -0.20453;
				break;
			}
	}
    pre_obj_place_pose = obj_place_pose;
	...
	new_obj_place_pose = obj_place_pose;
	new_obj_place_pose.pose.position.x = obj_place_pose.pose.position.x - 0.02;
...
{
    ros::init(argc, argv, "goback_demo");
    ros::NodeHandle node;

    ...
    def execute(self,userdata):
        try:
            if 'prefix' in userdata:
                ...
                raise ValueError('Jaco should be specified in userdata!')
            self.moveJoint(userdata)
        except Exception as e:
            rospy.logerr('Error when move to home: ' + repr(e))
            raise
        return 'succeeded'
...
class MotionPlanning(smach.State):
    ...
    def execute(self,userdata):
        try:
            ...
            rospy.logerr('Error when using move to joint positions: ' + repr(e))
            raise
        return 'succeeded'

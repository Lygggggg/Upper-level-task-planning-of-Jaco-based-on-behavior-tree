...
13: incremental EM HMM with new states''',
    ...
    parser.add_option('-r', '--repeat', dest='repeat', default=1, type='int',
                      help='repeat input')
    parser.add_option('--iter', dest='n_iter', default=30, type='int',
                      help='EM iterations')
    ...
    # matfile = loadmat("/home/lni/PycharmProjects/pythonProject/online_hmm/examples/ravel-fft.mat")
    data = np.loadtxt("/home/lni/PycharmProjects/pythonProject/Jaco-data-semg/data_processing/dataset_2021-04-06(1).csv")
    # data = data[:10220, 0:3].T
    data = data[12700:13850,0:3].T
    ...
    if options.init == 'kmeans' or algos.kmeans in algs:
        ...
        assignments, centroids, dists = \
                kmeans.kmeans_best_of_n(X, K, n_trials=4)
        # assignments, centroids, dists = \
    ...
    for alg in algs:
        if alg == algos.kmeans:
            ...
            # init_dur_distr = [distributions.NegativeBinomial(100, 0.1, D=200) for _ in range(K)]
            ...
            seqs[alg] = (seq_smoothing, seq_viterbi)
            state_list = list(seqs[4][0])  # seqs[alg] = (seq_smoothing) 变成list
            tmp_list = list(set(state_list))  # 去重后的list
            index = 0
            start = int(0)
            path = "/home/lni/Ros_ws/catkin_ws/src/hdp_hsmm/nodes/online_hmm/segment_data"
            for i in os.listdir(path):
                path_file = os.path.join(path, i)
                if os.path.isfile(path_file):
                    os.remove(path_file)
            for i in tmp_list:
                state_len = state_list.count(i)
                # with open(full_path,'w') as f:
                if state_len > 20:
                    full_path = "/home/lni/Ros_ws/catkin_ws/src/hdp_hsmm/nodes/online_hmm/segment_data/jiyuan_%d.txt" % index
                    file_ = open(full_path, 'w')
                    jiyuan_content = X[start:start+state_len, :]
                    start = start+state_len
                    # file_.write(jiyuan_content)
                    np.savetxt(full_path, jiyuan_content)
                    index += 1
                    file_.close()

            print "jiyuan_txt generated!,they are under /home/lni/Ros_ws/src/hdp_hsmm/nodes/online_hmm/segment_data/"
            ...
            # seq, obs_distr, cost = hmm.online_opt_hmm(X, 1.5, 250.0, init_obs_distr)
            ...
            t = time.time()
            tau, seq, A, obs_distr, dur_distr = hsmm.online_em_hsmm(X, init_pi, init_obs_distr, init_dur_distr, t_min=100, step=step)
